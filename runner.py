...
import random
from code_completion_baseline import CodeCompletionBaseline as CodeCompletion
from code_completion_improved1 import CodeCompletionImproved1 as CodeCompletion
...
model_file = "./../training_data/trained_model2"
...
def load_tokens(token_dir):
    token_files = \
        [join(token_dir, f) for f in listdir(token_dir) if isfile(join(token_dir, f)) and f.endswith("_tokens.json")]
...
for tokens in query_token_lists:
    ...
    print(prefix[-1], expected, completion)
